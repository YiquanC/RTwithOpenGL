...
using namespace glm;
 ...
 std::string rootPath = "D:/Learn/CG/Dev/EzRT/part3_OpenGLRaytracing/res/";
//表面物体材质定义
struct Material
{
...
void readObj(std::string filePath, std::vector<Triangle>& triangles, Material material, mat4 trans, bool smoothNormal)
{
    ...
    while (std::getline(fin,line))
    {
        ...
        if (type == "f")
        {
            if (slashCnt == 6)
            {
                ss >> v0 >> slash >> vt0 >> slash >> vn0;
                ss >> v1 >> slash >> vt1 >> slash >> vn1;
                ss >> v2 >> slash >> vt2 >> slash >> vn2;
            }
            else if (slashCnt == 3) 
            {
                ss>> v0 >> slash >> vt0;
    ...
    //生成法线
    std::vector<vec3> normals(vertices.size(), vec3(0));
...
int buildBVH(std::vector<Triangle>& triangles, std::vector<BVHNode>& nodes, int l, int r, int n)
{
    ...
    int id = nodes.size() - 1;      //保存索引 通过nodes[id]来操作
...
int main(int argc, char** argv)
{
    ...
    readObj(rootPath+"models/Stanford Bunny.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0.3, -1.6, 0), vec3(1.5, 1.5, 1.5)), true);
    ...
    readObj(rootPath+"models/quad.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, -1.4, 0), vec3(18.83, 0.01, 18.83)), false);

    m.baseColor = vec3(1, 0.8, 0.5);
    ...
    readObj(rootPath+"models/sphere.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(-2, -1.0, 0.0), vec3(1, 1, 1)), false);
    ...
    glBufferData(GL_TEXTURE_BUFFER, triangles_encoded.size() * sizeof(Triangle_encoded), triangles_encoded.data(), GL_STATIC_DRAW);
    ...
    glBufferData(GL_TEXTURE_BUFFER, nodes_encoded.size() * sizeof(BVHNode_encoded), nodes_encoded.data(), GL_STATIC_DRAW);
    ...
    bool r = HDRLoader::load((rootPath+"HDR/circus_arena_4k.hdr").c_str(), hdrRes);
    std::cout<<"HDR LOAD: "<<r<<std::endl;
    hdrMap = getTextureRGB32F(hdrRes.width, hdrRes.height);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, hdrRes.width, hdrRes.height, 0, GL_RGB, GL_FLOAT, hdrRes.cols);

    //-----------------------------------------------------------
    //管线配置
    pass1.program = getShaderProgram(rootPath+"shaders/fshader.glsl", rootPath + "shaders/vshader.glsl");
    ...
    pass2.program = getShaderProgram(rootPath + "shaders/fpass2.glsl", rootPath + "shaders/vshader.glsl");
    lastFrame = getTextureRGB32F(pass2.width, pass2.height);
    pass2.colorAttachments.push_back(lastFrame);
    pass2.bindData();

    pass3.program = getShaderProgram(rootPath + "shaders/fpass3.glsl", rootPath + "shaders/vshader.glsl");
    pass3.bindData(true);
